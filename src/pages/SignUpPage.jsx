...
export default function SignUpPage() {
  ...
  const isReady = values.username && values.email && values.password && !Object.values(errors).some(Boolean)

  const handleSubmit = async () => {
    if (!validate()) return
    setLoading(true)
    try {
      // ADDED: confirm_password because your backend requires it
      const signupData = {
        ...values,
      ...
      // FIXED: Handling the error object so React doesn't crash
      ...
      if (typeof errorDetail === 'string') {
        ...
        // If FastAPI sends technical details, we take the first message
      ...
      <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.6 }}
        ...
        <div className="bg-[#1c1518] border border-[rgba(255,255,255,0.08)] rounded-3xl p-8 shadow-card">
          <div className="text-center mb-8">
            <h1 className="font-display font-bold text-2xl text-[#F5F0EB] mb-1">Join SafeHer</h1>
            <p className="text-[#8a7a7d] text-sm">Become a Nighban — protect your community</p>
          </div>

          <button className="w-full flex items-center justify-center gap-3 py-3 rounded-xl border border-[rgba(255,255,255,0.1)] bg-white/[0.03] hover:bg-white/[0.06] text-[#F5F0EB] text-sm font-medium transition-all mb-6">
          ...
          <div className="space-y-4">
            <InputField label="Username" name="username" placeholder="e.g. ayesha_nighban"
              icon={User} value={values.username} onChange={set('username')} error={errors.username}
              hint="Letters, numbers and underscores only" required/>
            ...
            <InputField label="Password" name="password" type="password" placeholder="Min. 8 characters"
              icon={Lock} value={values.password} onChange={set('password')} error={errors.password}
            ...
            <div className="mt-3">
              <div className="h-1 bg-[rgba(255,255,255,0.07)] rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${
                  values.password.length < 6 ? 'w-1/4 bg-danger' :
                  values.password.length < 8 ? 'w-2/4 bg-yellow-400' :
                  values.password.length < 12 ? 'w-3/4 bg-safe' : 'w-full bg-safe'}`}/>
              </div>
              <p className="text-xs text-[#8a7a7d] mt-1">
                {values.password.length < 6 ? 'Weak' : values.password.length < 8 ? 'Fair' : values.password.length < 12 ? 'Good' : 'Strong'} password
              </p>
            </div>
          )}

          <motion.button onClick={handleSubmit} disabled={!isReady || loading}
          ...
          <p className="text-xs text-[#8a7a7d] text-center mt-4">
            By signing up you agree to our Terms of Service and Privacy Policy.
          </p>

          <p className="text-center text-sm text-[#8a7a7d] mt-5">
            Already have an account?{' '}
            <Link to="/signin" className="text-primary hover:text-accent transition-colors font-medium">Sign In</Link>
          </p>
        </div>
      </motion.div>
    </div>
